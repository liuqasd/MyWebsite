 ...
 */
if (!class_exists('Bigmart_Meta_Box')) {
    ...
    class Bigmart_Meta_Box {
        ...
        public function load_scripts_styles() {
            ...
            if (in_array($typenow, $this->_meta_box['pages']) && $this->is_edit_page()) {
                ...
                wp_enqueue_style('bigmart-meta-box', $plugin_path . '/assets/meta-box.css');

                // Enqueue Meta Box Scripts
                wp_enqueue_script('bigmart-meta-box', $plugin_path . '/assets/meta-box.js', array('jquery'), null, true);
         ...
         * @access public
         */
        public function show_field_repeater($field = array(), $meta) {
            global $post;
            // Get Plugin Path
            $plugin_path = $this->SelfPath;
            ...
            echo "<div class='bigmart-repeater" . $class . "' id='{$field['id']}'>";
            ...
            if (count($meta) > 0 && is_array($meta)) {
                foreach ($meta as $me) {
                    ...
                    if (in_array($field['fields'][0]['type'], array('image', 'file')))
                        $mmm = $c + 1;
                    echo '<div class="bigmart-repater-block">';
                    echo '<h6>';
                    if ($field['sortable']) {
                        echo '<span class="re-control bigmart-re-sort-handle"><i class="icofont-drag"></i></span>';
                    }
                    echo $mmm;
                    echo '<span class="re-control bigmart-re-toggle"><i class="icofont-simple-down"></i></span>';
                    echo '</h6>';
                    ...
                    echo '<span class="re-control bigmart-re-remove" id="remove-' . $field['id'] . '"><i class="icofont-close-circled"></i></span>';
                    echo '</div>';
                    echo '</div>';
                    $c = $c + 1;
                }
        ...
        public function show_field_text($field, $meta) {
            $this->show_field_begin($field, $meta);
            echo "<input type='text' class='bigmart-text" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='30' " . ( isset($field['style']) ? "style='{$field['style']}'" : '' ) . "/>";
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_number($field, $meta) {
            ...
            echo "<input type='number' class='bigmart-number" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='30' " . $step . $min . $max . ( isset($field['style']) ? "style='{$field['style']}'" : '' ) . "/>";
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_hidden($field, $meta) {
            echo "<input type='hidden' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . "class='bigmart-text" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' value='{$meta}'/>";
        }
        ...
        public function show_field_textarea($field, $meta) {
            $this->show_field_begin($field, $meta);
            echo "<textarea class='bigmart-textarea large-text" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " cols='60' rows='10'>{$meta}</textarea>";
            $this->show_field_end($field, $meta);
        }
         ...
         * @access public
         */
        public function show_field_select($field = array(), $meta) {
            if (!is_array($meta))
                $meta = (array) $meta;

            $this->show_field_begin($field, $meta);
            echo "<select " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='bigmart-select" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}" . ( $field['multiple'] ? "[]' id='{$field['id']}' multiple='multiple'" : "'" ) . ">";
         ...
         * @access public 
         */
        public function show_field_radio($field = array(), $meta) {
            if (!is_array($meta))
                $meta = (array) $meta;

            $this->show_field_begin($field, $meta);
            foreach ($field['options'] as $key => $value) {
                echo "<input type='radio' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='bigmart-radio" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' value='{$key}'" . checked(in_array($key, $meta), true, false) . " /> <span class='bigmart-radio-label'>{$value}</span>";
            }
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_image_radio($field, $meta) {
            ...
            $this->show_field_begin($field = array(), $meta);
            foreach ($field['options'] as $key => $value) {
                echo '<label class="ms-meta-box-image-select">';
                echo "<img src='{$value}'>";
                echo "<input type='radio' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='bigmart-radio" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' value='{$key}'" . checked(in_array($key, $meta), true, false) . " />";
                echo "<span></span>";
                echo "</label>";
            }
            $this->show_field_end($field, $meta);
        ...
        public function show_field_wysiwyg($field, $meta, $in_repeater = false) {
            ...
            if ($in_repeater)
                echo "<textarea class='bigmart-wysiwyg theEditor large-text" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>{$meta}</textarea>";
            else {
                $settings = ( isset($field['settings']) && is_array($field['settings']) ? $field['settings'] : array() );
                $settings['editor_class'] = 'bigmart-wysiwyg' . ( isset($field['class']) ? ' ' . $field['class'] : '' );
        ...
        public function show_field_color($field, $meta) {
            $this->show_field_begin($field, $meta);
            echo "<input class='bigmart-color-iris" . (isset($field['class']) ? " {$field['class']}" : "") . "' type='text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='8' />";
        ...
        public function show_field_alpha_color($field, $meta) {
            $this->show_field_begin($field, $meta);
            echo "<input data-alpha-enabled='true' class='bigmart-color-iris" . (isset($field['class']) ? " {$field['class']}" : "") . "' type='text' name='{$field['id']}' id='{$field['id']}' value='{$meta}' />";
         ...
         * @access public
         */
        public function show_field_checkbox_list($field = array(), $meta) {
            if (!is_array($meta))
                $meta = (array) $meta;
            ...
            foreach ($field['options'] as $key => $value) {
                ...
                echo "<input type='checkbox' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . "  class='bigmart-checkbox_list" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}[]' value='{$key}'" . checked(in_array($key, $meta), true, false) . " /> {$value}";
                echo "</div>";
            }
        ...
        public function show_field_date($field, $meta) {
            $this->show_field_begin($field, $meta);
            echo "<input type='text'  " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='bigmart-date" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='{$meta}' size='30' />";
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_time($field, $meta) {
            ...
            echo "<input type='text'  " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='bigmart-time" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}' id='{$field['id']}' data-ampm='{$ampm}' rel='{$field['format']}' value='{$meta}' size='30' />";
            $this->show_field_end($field, $meta);
        }
        ...
        public function show_field_posts($field, $meta) {
            ...
            if ('checkbox_list' == $options['type']) {
                foreach ($posts as $p) {
                    echo "<input type='checkbox' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='bigmart-posts-checkbox" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}[]' value='$p->ID'" . checked(in_array($p->ID, $meta), true, false) . " /> $p->post_title<br/>";
                }
            }
            // select
            else {
                echo "<select " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='bigmart-posts-select" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}" . ($field['multiple'] ? "[]' multiple='multiple' style='height:auto'" : "'") . ">";
                if (isset($field['emptylabel']))
                    echo '<option value="-1">' . (isset($field['emptylabel']) ? $field['emptylabel'] : __('Select ...', 'bigmart')) . '</option>';
                foreach ($posts as $p) {
        ...
        public function show_field_taxonomy($field, $meta) {
            ...
            if ('checkbox_list' == $options['type']) {
                foreach ($terms as $term) {
                    echo "<input type='checkbox' " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='bigmart-tax-checkbox" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}[]' value='$term->slug'" . checked(in_array($term->slug, $meta), true, false) . " /> $term->name<br/>";
                }
            }
            // select
            else {
                echo "<select " . ( isset($field['style']) ? "style='{$field['style']}' " : '' ) . " class='bigmart-tax-select" . ( isset($field['class']) ? ' ' . $field['class'] : '' ) . "' name='{$field['id']}" . ($field['multiple'] ? "[]' multiple='multiple' style='height:auto'" : "'") . ">";
                foreach ($terms as $term) {
        ...
        public function save($post_id) {
            ...
            foreach ($this->_fields as $field) {
                ...
                // Validate meta value
                if (class_exists('bigmart_Meta_Box_Validate') && method_exists('bigmart_Meta_Box_Validate', $field['validate_func'])) {
                    $new = call_user_func(array('bigmart_Meta_Box_Validate', $field['validate_func']), $new);
                }
         ...
         * @access public 
         */
        public function save_field_repeater($post_id, $field = array(), $old, $new) {
         ...
         * @access public
         */
        public function fix_file_array(&$files = array()) {
         ...
         *    'args' =>  // arguments to query taxonomy,
         *  @param $args mixed|array
         *    'name' => // field name/label string optional
         *    'desc' => // field description, string optional
         *    'std' => // default value, string optional
